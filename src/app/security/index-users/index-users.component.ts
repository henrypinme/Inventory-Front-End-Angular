...
@Component({
  ...
  imports: [ MatButtonModule, MatTableModule, MatPaginatorModule, SweetAlert2Module, GenericListComponent],
...
export class IndexUsersComponent {
  ...
  pagination: PaginationDTO = {page: 1, recordPerPage: 10};
  ...
  loadRecords(){
    this.securityService.getUserPaginated(this.pagination).subscribe((response: HttpResponse<UserDTO[]>) => {
  ...
  makeAdmin(email: string){
    this.securityService.makeAdmin(email).subscribe(() => {
      Swal.fire("Successful", `The user ${email} is now admin`, 'success');
    })
  }
